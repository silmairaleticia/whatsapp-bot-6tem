...
const LEADS_FILE = path.join(DATA_DIR, 'quiz_leads.json');
...
// Lead capturado no quiz de qualificação, antes da pessoa ir pro checkout.
export function addQuizLead(lead) {
      const leads = loadLeads();
      leads.push({ ...lead, timestamp: Date.now() });
      saveLeads(leads);
}

export function listQuizLeads() {
      return loadLeads()
        .slice()
